...
import {
  Loader2, CheckCircle, XCircle, Circle, PauseCircle, ChevronDown, ChevronRight, GitPullRequest,
} from '@/lib/lucide-icons';
import type { AnalysisStage } from '../services/types';

export type AnalyzeStageStatus = 'pending' | 'running' | 'checkpoint' | 'completed' | 'error';

export interface AnalyzeStageInfo {
  stage: AnalysisStage;
  status: AnalyzeStageStatus;
  toolCalls: { tool: string; summary: string }[];
}

interface AnalyzeTraceDrawerProps {
  stages: AnalyzeStageInfo[];
  analyzing: boolean;
...
const STAGE_LABELS: Record<AnalysisStage, string> = {
  ...
  unit_tests: 'Unit Test Generation',
  checkpoint_unit: 'Unit Test Review',
  choice: 'Stage Selection',
  integration_tests: 'Integration Tests',
  e2e_checkpoint: 'E2E Context',
...
function StatusIcon({ status }: { status: AnalyzeStageStatus }) {
  switch (status) {
    case 'running':
...
function StageRow({ info, isActive }: { info: AnalyzeStageInfo; isActive: boolean }) {
  const [expanded, setExpanded] = useState(false);
  const rowRef = useRef<HTMLDivElement>(null);
  const canExpand = (info.status === 'running' || info.status === 'completed' || info.status === 'checkpoint') && info.toolCalls.length > 0;
...
export const AnalyzeTraceDrawer = ({ stages, analyzing, currentStage }: AnalyzeTraceDrawerProps) => {
  const stageMap = new Map(stages.map(s => [s.stage, s]));
  const totalCalls = stages.reduce((sum, s) => sum + s.toolCalls.length, 0);

  const rows: AnalyzeStageInfo[] = STAGE_ORDER.map(stage =>
    ...
    <div className="flex h-full flex-col bg-surface">
      ...
      <div className="shrink-0 border-b border-border-subtle px-3 py-2.5">
        <div className="flex items-center gap-2.5 rounded-lg border border-accent/20 bg-accent/10 px-3 py-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-accent">PR Analysis</p>
            {currentStage && (
              <p className="text-[10px] text-text-muted">{STAGE_LABELS[currentStage]}</p>
            )}
          </div>
          {analyzing && <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-accent" />}
